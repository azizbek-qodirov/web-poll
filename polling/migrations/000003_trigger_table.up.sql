...
CREATE OR REPLACE FUNCTION insert_poll(p_title VARCHAR, p_options JSONB) RETURNS VOID AS $$
DECLARE
    next_num INT;
BEGIN
    next_num := get_next_poll_num();
    INSERT INTO polls (poll_num, title, options) VALUES (next_num, p_title, p_options);
END;
$$ LANGUAGE plpgsql;
